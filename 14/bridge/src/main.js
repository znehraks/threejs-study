...
const pillar2 = new Pillar({
  ...
  x: 0,
  y: 5.5,
  z: glassUnitSize * 13 - glassUnitSize / 2,
});
...
let glassTypes = [];
const glassZ = [];
for (let i = 0; i < numberOfGlasses; i += 1) {
...
objects.push(player, pillar1, pillar2);
// Raycaster
      ...
      switch (mesh.type) {
        case 'normal':
          ...
          setTimeout(() => {
            ...
            sideLights.forEach((s) => s.turnOff());
            spotLight1.color.set('#222');
            spotLight2.color.set('#222');
            setTimeout(() => {
              onReplay = true;
