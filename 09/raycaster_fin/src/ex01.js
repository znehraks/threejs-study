...
export default function example() {
  ...
  const from = new THREE.Vector3(2, 2, 2);
  ...
  const rFrom = new THREE.Vector3(0, 0, 0);
  const rTo = new THREE.Vector3(2, 2, 2);
  const rDirection = rTo.clone().sub(rFrom);
  const rLength = rDirection.length();
  const rArrowHelper = new THREE.ArrowHelper(
    rDirection.normalize(),
    rFrom,
    rLength,
    0xfd93df
  );
  scene.add(rArrowHelper);
